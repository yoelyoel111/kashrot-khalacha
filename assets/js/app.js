  ...
  container.innerHTML = approved.map((q, index) => `
    <div class="question-card fade-up" data-question-id="${q.id}">
      ...
      <div class="card-details-line">
        ...
        ${q.hechsher ? `<span class="detail-separator">|</span><span style="color: #8B4513; font-weight: 600;">${escapeHtml(q.hechsher)}</span>` : q.kashrutType ? `<span class="detail-separator">|</span><span style="color: #8B4513; font-weight: 600;">${escapeHtml(q.kashrutType)}</span>` : ''}
      </div>
      <div class="card-question-preview">${escapeHtml(q.question?.substring(0, 120) || 'אין פירוט')}${q.question?.length > 120 ? '...' : ''}</div>
      <div class="card-meta">
        <span>${formatDate(q.createdAt)}</span>
      </div>
      <div class="card-actions">
        <button class="btn btn-primary" onclick="openQuestionDetail('${q.id}', ${index})">פתח שאלה</button>
      </div>
    </div>
  `).join('');
...
function applyFilters() {
  ...
  const filtered = questions.filter(q => {
    ...
    if (text && !q.question.toLowerCase().includes(text) && !q.answer.toLowerCase().includes(text)) return false;
...
function handleClickOutside(e) {
  const detail = document.getElementById('question-detail');
  if (!detail || detail.getAttribute('data-open') !== 'true') return;
  
  // Check if click is outside the detail panel
  if (!detail.contains(e.target) && !e.target.closest('.question-card')) {
    closeQuestionDetail();
...
function highlightCurrentQuestionCard() {
  // Remove current-viewing class from all cards
  document.querySelectorAll('.question-card').forEach(card => {
    card.classList.remove('current-viewing');
  });
  ...
  if (currentQuestionIndex >= 0 && currentQuestionList[currentQuestionIndex]) {
    ...
    if (currentCard) {
      currentCard.classList.add('current-viewing');
...
function nextQuestion() {
  if (currentQuestionIndex < currentQuestionList.length - 1) {
    ...
    openQuestionDetail(nextQ.id, currentQuestionIndex);
    
    // Highlight and scroll to the new current question
    highlightCurrentQuestionCard();
  }
}
...
function previousQuestion() {
  if (currentQuestionIndex > 0) {
    ...
    openQuestionDetail(prevQ.id, currentQuestionIndex);
